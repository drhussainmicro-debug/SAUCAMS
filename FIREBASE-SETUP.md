...
Not required — but it fixes one thing GitHub Pages cannot. Pages serves
`index.html` with a fixed 10-minute cache, so a returning user may not see a new
build for up to 10 minutes. `firebase.json` in this repo sets `no-cache` on the
HTML and the service worker, so updates land on the next visit.

```
npm install -g firebase-tools
firebase login
firebase use --add            # pick the sauc-ams project
firebase deploy --only hosting,firestore:rules
```

That also deploys `firestore.rules`, so you can stop pasting them into the
console by hand. You can keep GitHub Pages running at the same time; they are
independent.
